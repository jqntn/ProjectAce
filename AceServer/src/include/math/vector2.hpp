...
struct Vector2
{
  ...
  // Calcule l'angle (en degrés) entre deux directions (vecteurs normalisés) en
  // degrés
  ...
  static Vector2 Normalize(const Vector2& vec) { return vec / vec.Length(); }
  ...
  bool operator!=(const Vector2& vec) const { return !operator==(vec); }
};

// float * vec2
inline Vector2
operator*(float n, const Vector2& vec)
{
...
inline std::ostream&
operator<<(std::ostream& oss, const Vector2& vec)
{
