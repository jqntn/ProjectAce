...
template<class T>
constexpr T
Clamp(const T X, const T MinValue, const T MaxValue)
{
...
/** Interpolate float from Current to Target. Scaled by distance to Target, so
 * it has a strong start speed and ease out. */
template<typename T1, typename T2 = T1, typename T3 = T2, typename T4 = T3>
auto
SmoothLerp(T1 Current, T2 Target, T3 DeltaTime, T4 InterpSpeed)
{
  ...
  const RetType DeltaMove =
    Dist * Clamp<RetType>(DeltaTime * InterpSpeed, 0.f, 1.f);
