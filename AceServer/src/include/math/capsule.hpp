...
struct Capsule
{
  ...
  {
    // Est-ce que la distance (au carré) entre le segment formé par la capsule
    // et la position de la sphère est inférieur au radius de la capsule et de
    // la sphère (au carré)
    float distSq =
      Vector3::SegmentDistance(capsule.start, capsule.end, sphere.center);
  ...
  {
    // Est-ce que la distance (au carré) entre les deux segments est inférieur à
    // la somme des radius au carré
    float d1, d2;
    Vector3 p1, p2;
    float distSq =
      Vector3::ClosestPoint(c1.start, c1.end, c2.start, c2.end, d1, d2, p1, p2);
  ...
  {
    return start == capsule.start && end == capsule.end &&
           radius == capsule.radius;
  }

  bool operator!=(const Capsule& capsule) const { return !operator==(capsule); }
};

inline std::ostream&
operator<<(std::ostream& oss, const Capsule& capsule)
{
  return oss << "Capsule(" << capsule.start << " -> " << capsule.end << "; "
             << capsule.radius << ")";
}