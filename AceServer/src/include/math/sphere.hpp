...
struct Sphere
{
  ...
  // Calcule la distance entre la surface de deux sphères (négatif si
  // intersection): float d = Sphere::Distance(s1, s2)
  static float Distance(const Sphere& s1, const Sphere& s2)
  {
    return Vector3::Distance(s1.center, s2.center) - s1.radius - s2.radius;
  }

  // Renvoie vrai si les sphères sont en intersection: if
  // (Sphere::Intersects(s1, s2))
...
inline std::ostream&
operator<<(std::ostream& oss, const Sphere& sphere)
{
