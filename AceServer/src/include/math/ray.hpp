...
struct Ray
{
  ...
  // Renvoie le point situé à une certaine distance sur ce rayon: Vector3 p =
  // ray.At(d)
  Vector3 At(float dist) const { return origin + dist * direction; }

  // Calcule la distance minimale entre un rayon et une position: float f =
  // Ray::Distance(r, v)
  ...
  // Renvoie vrai en cas d'intersection entre un rayon et une boite (ainsi que
  // la distance d'entrée/sortie) La distance peut être négative si le rayon
  // démarre à l'intérieur de la box
  static bool Intersects(const Ray& r, const Box& b, float& tmin, float& tmax)
  {
    ...
    tmin = std::max(std::max(std::min(t0s.x, t1s.x), std::min(t0s.y, t1s.y)),
                    std::min(t0s.z, t1s.z));
    tmax = std::min(std::min(std::max(t0s.x, t1s.x), std::max(t0s.y, t1s.y)),
                    std::max(t0s.z, t1s.z));
    if (tmax < 0.f)
  ...
  // Renvoie vrai en cas d'intersection entre une capsule infinie (rayon avec
  // radius) et une sphère: if (Ray::Intersects(r, r_radius, s, &intersection))
  static bool Intersects(const Ray& r,
                         float rayRadius,
                         const Sphere& s,
                         Vector3* intersection = nullptr)
  {
  ...
  // Renvoie vrai en cas d'intersection entre un rayon et une capsule: if
  // (Ray::Intersects(r, c, &p1, &p2, &n1, &n2) La fonction sort les deux points
  // d'intersection (entrée/sortie) ainsi que les deux normales de collision
                         ...
                         Vector3& n2)
  {
    ...
    if (a < Epsilon<float>) {
      // Special case: AB and ray direction are parallel. If there is an
      // intersection it will be on the end spheres... NOTE: Why is that? Q = d
      // - AB * m => Q = d - AB * (|AB|*|d|*cos(AB,d) / |AB|^2) => |d| == 1.0 Q
      // = d - AB * (|AB|*cos(AB,d)/|AB|^2) => Q = d - AB * cos(AB, d) / |AB| =>
      // Q = d - unit(AB) * cos(AB, d)
      //
      // |Q| == 0 means Q = (0, 0, 0) or d = unit(AB) * cos(AB,d)
      // both d and unit(AB) are unit vectors, so cos(AB, d) = 1 => AB and d are
      // parallel.
      //
      ...
      if (!Intersects(ray, sphereA, atmin, atmax) ||
        ...
        // No intersection with one of the spheres means no intersection at
        // all...
        return false;
      }
  ...
  // Renvoie vrai en cas d'intersection entre un rayon et une sphère: if
  // (Ray::Intersects(r, s, &intersection))
  static bool Intersects(const Ray& r,
                         const Sphere& s,
                         Vector3* intersection = nullptr)
  {
  ...
  // Renvoie vrai en cas d'intersection entre un rayon et une sphère: if
  // (Ray::Intersects(r, s, &intersection)) Renvoie les deux distances
  // d'intersection (entrée et sortie)
  static bool Intersects(const Ray& r,
                         const Sphere& s,
                         float& tmin,
                         float& tmax)
...
inline std::ostream&
operator<<(std::ostream& oss, const Ray& ray)
{
