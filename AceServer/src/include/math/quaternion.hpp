...
struct Quaternion
{
  ...
  // Applique la rotation du quaternion à un vecteur: Vec3 rotatedVec =
  // quat.Rotate(vec);
  ...
  // Calcule la rotation inverse d'un quaternion normalisé: Quaternion invQuat =
  ...
  // Calcule la rotation permettant de passer d'une direction (vecteur
  // normalisé) é une autre: Quaternion quat = Quaternion::RotationBetween(from,
  // to)
  ...
  // Calcule un quaternion normalisé: Quaternion normalizedQuat =
...
inline std::ostream&
operator<<(std::ostream& oss, const Quaternion& quat)
{
  return oss << "Quaternion(" << quat.x << "; " << quat.y << "; " << quat.z
             << " | " << quat.w << ")";
}