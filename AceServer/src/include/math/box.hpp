...
// Classe gérant des boites alignées (sans rotation), avec un centre et des
// dimensions
struct Box
{
  Vector3 center;
  ...
  // Renvoie le point minimal (souvent considéré comme origine dans certains
  // calculs/moteurs)
  Vector3 Mins() const { return center - dims * 0.5f; }

  // Renvoie le point maximal
  Vector3 Maxs() const { return center + dims * 0.5f; }

  // Construit une boite plus classique depuis un point d'origine et des
  // dimensions
  ...
  // Renvoie vrai si les boites sont en intersection: if (Box::Intersects(b1,
  // b2)) Calcule de façon optionnelle la boite de collision
  static bool Intersects(const Box& b1,
                         const Box& b2,
                         Box* intersection = nullptr)
  {
    ...
    if (intersection)
      *intersection = FromOriginDims(
        Vector3(left, top, up), Vector3(right - left, bottom - top, down - up));
  ...
  bool operator!=(const Box& box) const { return !operator==(box); }
};

inline std::ostream&
operator<<(std::ostream& oss, const Box& box)
{
